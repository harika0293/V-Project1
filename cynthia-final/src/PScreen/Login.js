        ...
        db.collection('usersCollections')
          ...
          .then(function (user) {
            if (user.exists) {
              ...
              AsyncStorage.setItem('@loggedInUserID:profile', user.data().profile?user.data().profile:'Not Updated');
              AsyncStorage.setItem('@loggedInUserID:onboarded', "true");
              user.data().id = user_uid
              dispatch(login(user.data()));
              user.data().profile === 'Updated'?navigation.navigate('BottomNavigator'):navigation.navigate('AuthStack', { screen: 'PEditProfile' });
            } else {
              setLoading(false);
  ...
  const onPressGoogle = () => {
    GoogleSignin.signIn()
      ...
      db.collection('usersCollections').doc(user.uid).set(data);
        ...
        profile === 'Updated'?navigation.navigate('BottomNavigator', {user}):navigation.navigate('AuthStack', { screen: 'PEditProfile', params: { user} });
      })
      .catch((error) => {
        const {message} = error;
        setLoading(false);
        ...
        <Layout style={styles.signin}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('OTPScreen')}>
            <Text style={{fontSize: 17, color: '#0075A9', fontWeight: 'bold'}}>
              {' '}
              Create Your Account As A Patient
            </Text>
          </TouchableOpacity>
        </Layout>
